...
/** Submit premium-mentor payment screenshot for admin verification. */
router.post(
  "/premium-mentor",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (user.role !== "faculty") {
        res.status(403).json({ error: "Only mentors can apply for Premium" });
        return;
      }
      if (user.premiumMentorStatus === "verified") {
        res.status(400).json({ error: "Already a Premium Mentor" });
        return;
      }
      ...
      if (imageBase64.length > 12_000_000) {
        res.status(400).json({ error: "Image too large — use a smaller screenshot" });
        return;
      }

      const { uploadPremiumPaymentScreenshot } = await import(
        "../services/premium-mentor"
      );
      const uploaded = await uploadPremiumPaymentScreenshot(
        user._id.toString(),
        user.premiumMentorPaymentSsPath,
      ...
      user.premiumMentorStatus = "pending";
      user.premiumMentorPaymentSsUrl = uploaded.url;
      user.premiumMentorPaymentSsPath = uploaded.path;
      user.premiumMentorSubmittedAt = new Date();
      user.premiumMentorVerifiedAt = undefined;
      user.set("premiumMentorVerifiedAt", undefined);
      await user.save();

      res.json({
        user: serializeUser(user),
        message: "Payment screenshot submitted — pending verification",
      });
    } catch (error) {
      console.error("premium mentor submit error:", error);
      res.status(500).json({ error: "Failed to submit payment screenshot" });
    }
  },
);
